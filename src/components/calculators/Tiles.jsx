...
const THEME = THEME_COLORS.tiles;
...
export default function Tiles() {
    ...
    // Updates material_type and resets tile_size_cm atomically in one state update
    const handleMaterialChange = (id, materialId) => {
        setRows(prev => prev.map(r => r.id === id ? { ...r, material_type: materialId, tile_size_cm: '' } : r));
        setResult(null);
    };
    ...
    return (
        <div className="space-y-6">
            ...
            <Card className="border-t-4 shadow-md bg-white rounded-xl" style={{ borderTop: '4px solid #059669' }}>
                <SectionHeader
                    title="Floor Tiles Area Configuration"
                    icon={LayoutGrid}
                    colorTheme={THEME}
                    actions={
                        <ActionButton
            ...
            {!result && !error && (
                <div className="border-2 border-dashed border-slate-300 rounded-xl p-16 flex flex-col items-center justify-center text-center text-slate-400 bg-slate-50/50 mt-6">
                    <div className="bg-white p-4 rounded-full shadow-sm mb-4">
                        <LayoutGrid size={40} className={`text-${THEME}-400`} />
                    </div>
                    <h3 className="text-lg font-bold text-slate-600 mb-1">Ready to Estimate</h3>
                    <p className="max-w-md mx-auto text-sm">
                        Enter your tile area dimensions and specifications above, then click <span className={`font-bold text-${THEME}-600`}>'CALCULATE'</span>.
                    </p>
                </div>
            )}
                        ...
                        <div className="flex flex-col md:flex-row justify-between md:items-start mb-8 gap-6">
                            <div>
                                ...
                                <div className="flex flex-wrap gap-4 mt-3">
                                    <p className="text-sm text-gray-500">Total Floor Area: <strong className="text-gray-900">{result.totalArea.toFixed(2)} m²</strong></p>
                                </div>
                            </div>
