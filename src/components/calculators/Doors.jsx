...
import { calculateDoorsWindows, itemTypes, frameMaterials, groupedFrameOptions, leafMaterials, groupedLeafOptions, defaultMaterialMap } from '../../utils/calculations/doorsWindowsCalculator';
...
export default function Doors() {
    const [items, setItems] = useLocalStorage('doors_rows', [getInitialItem()]);
    const [prices, setPrices] = useLocalStorage('app_material_prices', getDefaultPrices(), { mergeDefaults: true });
    const [result, setResult] = useLocalStorage('doors_result', null);
    ...
    useEffect(() => {
        if (result) {
            setSessionData('doors_total', result.grandTotal);
        } else {
            setSessionData('doors_total', null);
        }
                                ...
                                <tr key={item.id} className={`${TABLE_UI.INPUT_ROW} ${item.isExcluded ? 'opacity-50 grayscale-[0.5]' : ''}`}>
                                    ...
                                    <td className={TABLE_UI.INPUT_CELL}>
                                        <SelectInput
                                            ...
                                            options={itemTypes.filter(g => g.group === 'Doors')}
                                    ...
                                    <td className={TABLE_UI.INPUT_CELL}>
                                        <SelectInput
                                            ...
                                            options={groupedLeafOptions.filter(g => g.group !== 'Louver/Jalousie').map(g => ({ ...g, options: g.options.filter(opt => !opt.id.toLowerCase().match(/casement|awning|jalousie|louver|window/)) })).filter(g => g.options.length > 0)}
            ...
            {!result && !error && (
                <div className="border-2 border-dashed border-slate-300 rounded-xl p-16 flex flex-col items-center justify-center text-center text-slate-400 bg-slate-50/50">
                    ...
                    <p className="max-w-md mx-auto text-sm">
                        Enter your door specifications above, then click <span className={`font-bold text-${THEME}-600`}>'CALCULATE'</span>.
                    </p>
                </div>
            )}
                        ...
                        <div className="flex flex-col md:flex-row justify-between md:items-start mb-8 gap-6">
                            ...
                            <div className="flex flex-col items-end gap-3">
                                ...
                                <div className="flex gap-2">
                                    <ExportButtons items={result.items} filename="doors_estimate.csv" />
                                </div>
                            </div>
                        </div>
